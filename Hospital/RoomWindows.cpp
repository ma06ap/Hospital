...
void DoctorWindow::closeEvent(QCloseEvent *event) {
    if (parentWidget()) {
        parentWidget()->show(); // باز کردن مجدد صفحه اصلی
    }
    event->accept();
}
...
void DoctorWindow::setupUi() {
    ...
    // --- Middle: Medicine List for Selected Patient ---
...
void DoctorWindow::refreshPatientTable() {
    ...
    // گرفتن بیماران داخل اتاق (Active)
    ...
    for (auto p : patients) {
        ...
        item->setData(Qt::UserRole, QVariant::fromValue((void*)p)); // ذخیره پوینتر
...
void DoctorWindow::onFinalizePrescription() {
    ...
    // 1. انتقال بیمار از اتاق دکتر به بیرون
    // متد removePatient به طور خودکار نفر بعدی را از صف انتظار به لیست فعال می‌آورد
    room->removePatient(currentSelectedPatient);

    // 2. افزودن به داروخانه
...
void DoctorWindow::onUpdateQueue() {
    // این دکمه صرفاً برای رفرش کردن لیست است
    // چون در طراحی ما، کلاس Room به صورت خودکار وقتی کسی خارج شود،
    // نفر بعدی را جایگزین می‌کند.
    refreshPatientTable();
}


// =========================================================
//  PHARMACY WINDOW IMPLEMENTATION
// =========================================================
...
void PharmacyWindow::refreshTable() {
    ...
    // در داروخانه همه در لیست فعال هستند (صف بیرونی نداریم طبق کانسپت)
    auto patients = room->getActivePatients();
...
void PharmacyWindow::showMedicineDialog(Patient* p) {
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        room->removePatient(p); // خروج از داروخانه
...
// =========================================================
//  INJECTION WINDOW IMPLEMENTATION
// =========================================================
...
void InjectionWindow::showInjectionDialog(Patient* p) {
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        room->removePatient(p); // خروج از تزریقات
